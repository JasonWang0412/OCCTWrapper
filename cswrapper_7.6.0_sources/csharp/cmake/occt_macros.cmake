...
macro (OCCT_CHECK_AND_UNSET_INSTALL_DIR_SUBDIRS)
  ...
  OCCT_CHECK_AND_UNSET (INSTALL_DIR_TRAINING)
...
macro (OCCT_INSTALL_FILE_OR_DIR BEING_INSTALLED_OBJECT DESTINATION_PATH COMPONENT_NAME)
  if (BUILD_PATCH AND EXISTS "${BUILD_PATCH}/${BEING_INSTALLED_OBJECT}")
    if (IS_DIRECTORY "${BUILD_PATCH}/${BEING_INSTALLED_OBJECT}")
      ...
      install (DIRECTORY "${CMAKE_SOURCE_DIR}/${BEING_INSTALLED_OBJECT}" DESTINATION  "${DESTINATION_PATH}" COMPONENT ${COMPONENT_NAME})

      # secondly, rewrite original files with patched ones
      install (DIRECTORY "${BUILD_PATCH}/${BEING_INSTALLED_OBJECT}" DESTINATION  "${DESTINATION_PATH}" COMPONENT ${COMPONENT_NAME})
    else()
      install (FILES     "${BUILD_PATCH}/${BEING_INSTALLED_OBJECT}" DESTINATION  "${DESTINATION_PATH}" COMPONENT ${COMPONENT_NAME})
    endif()
  else()
    if (IS_DIRECTORY "${CMAKE_SOURCE_DIR}/${BEING_INSTALLED_OBJECT}")
      install (DIRECTORY "${CMAKE_SOURCE_DIR}/${BEING_INSTALLED_OBJECT}" DESTINATION  "${DESTINATION_PATH}" COMPONENT ${COMPONENT_NAME})
    else()
      install (FILES     "${CMAKE_SOURCE_DIR}/${BEING_INSTALLED_OBJECT}" DESTINATION  "${DESTINATION_PATH}" COMPONENT ${COMPONENT_NAME})
    endif()
  endif()
endmacro()

macro (OCCT_CONFIGURE_AND_INSTALL BEING_CONGIRUGED_FILE BUILD_NAME INSTALL_NAME DESTINATION_PATH COMPONENT_NAME)
  ...
  install(FILES "${PROD_BINARY_DIR}/${BUILD_NAME}" DESTINATION  "${DESTINATION_PATH}" RENAME ${INSTALL_NAME} COMPONENT ${COMPONENT_NAME})
endmacro()
...
macro (COLLECT_AND_INSTALL_OCCT_HEADER_FILES ROOT_TARGET_OCCT_DIR OCCT_BUILD_TOOLKITS)
        ...
        foreach (FILE_INDEX RANGE ${ALL_FILES_NB})
          ...
          if ("${OCCT_FILE_IN_DIR_NAME}" STREQUAL "${OCCT_FILE_NAME}")
            ...
            if (IS_HEADER_FOUND)
              ...
              if (BUILD_MODULE_OMF AND BUILD_ARCHIVE_DELIVERY)
                if (${OCCT_PACKAGE} STREQUAL "Geodesic" OR 
                    ${OCCT_PACKAGE} STREQUAL "PolyAlgo" OR
                    ${OCCT_PACKAGE} STREQUAL "Poly2d" OR
                    ${OCCT_PACKAGE} STREQUAL "GeomConv2d" OR
                    ${OCCT_PACKAGE} STREQUAL "QMBgr" OR
                    ${OCCT_PACKAGE} STREQUAL "QMData" OR
                    ${OCCT_PACKAGE} STREQUAL "QMShape" OR
                    ${OCCT_PACKAGE} STREQUAL "QMTools")
                  list (APPEND OCCT_HEADER_FILES_NOT_INSTALL ${OCCT_FILE_IN_DIR})
                endif()
              endif()
  ...
  if (BUILD_ARCHIVE)
    list (REMOVE_ITEM OCCT_HEADER_FILES_COMPLETE "${CMAKE_SOURCE_DIR}/src/OCCLicense/OCCLicense.hxx" "${CMAKE_SOURCE_DIR}/src/OCCLicense/OCCLicense_Tools.hxx" "${CMAKE_SOURCE_DIR}/src/OCCLicense/OCCLicense_ToolsSrc.hxx" "${CMAKE_SOURCE_DIR}/src/OCCLicense/iptypes.h")
    if (BUILD_MODULE_OMF AND BUILD_ARCHIVE_DELIVERY)
      foreach (HEADER_NOT_INSTALL ${OCCT_HEADER_FILES_NOT_INSTALL})
        list (REMOVE_ITEM OCCT_HEADER_FILES_COMPLETE "${HEADER_NOT_INSTALL}")
      endforeach()
    endif()
  endif()
  install (FILES ${OCCT_HEADER_FILES_COMPLETE} DESTINATION "${INSTALL_DIR_INCLUDE}" COMPONENT myheaders)
...
function (OCC_VERSION OCC_VERSION_MAJOR OCC_VERSION_MINOR OCC_VERSION_MAINTENANCE OCC_VERSION_DEVELOPMENT OCC_VERSION_STRING_EXT)
  ...
  set (STANDARD_VERSION_FILE "${OpenCASCADE_INCLUDE_DIR}/Standard_Version.hxx")
...
# "i" for RelWithDebInfo) in PRODUCTSTargets-*.cmake files during 
...
macro (OCCT_UPDATE_TARGET_FILE)
  ...
  file (GLOB ALL_OCCT_TARGET_FILES \"${INSTALL_DIR}/${INSTALL_DIR_CMAKE}/PRODUCTS*Targets-\${CMAKE_INSTALL_CONFIG_NAME_LOWERCASE}.cmake\")
...
macro (OCCT_UPDATE_DRAW_DEFAULT_FILE)
  ...
  cmake_policy(SET CMP0007 NEW)
  set (DRAW_DEFAULT_FILE_NAME \"${INSTALL_DIR}/${INSTALL_DIR_RESOURCE}/DrawResourcesProducts/DrawPluginProducts\")
...
# function to find necessary files for TOOLKITS
function (HEADER_FIND OCCT_TOOLKIT)
  set (TEST_PACKAGE_LIST)
  FILE_TO_LIST ("src/${OCCT_TOOLKIT}/PACKAGES" USED_PACKAGES)
  # create relative path to install directory
  string (REPLACE "${INSTALL_DIR}/" "" RELATIVE_INSTALL_INCLUDE_DIR "${INSTALL_DIR_INCLUDE}")
  foreach (PACKAGE ${USED_PACKAGES})
    # find cxx and hxx for test\draw toolkits (in src folder)
    string (REGEX MATCH "TK" TK_FOUND ${PACKAGE})
    if ("${TK_FOUND}" STREQUAL "")
      foreach (TEST_PACK "Test" "DRAW")
        string (REGEX MATCH ".*${TEST_PACK}.*" TEST_PACKAGE ${PACKAGE})
        if (NOT "${TEST_PACKAGE}" STREQUAL "")
          list (APPEND TEST_PACKAGE_LIST ${TEST_PACKAGE})
          file (STRINGS "${CMAKE_SOURCE_DIR}/src/${TEST_PACKAGE}/FILES"     TEST_FILES_CXX REGEX ".+[.]cxx")
          file (STRINGS "${CMAKE_SOURCE_DIR}/src/${TEST_PACKAGE}/FILES"     TEST_FILES_HXX REGEX ".+[.]h")
          foreach (TEST_FILE_CXX ${TEST_FILES_CXX})
            list (APPEND TEST_FILES_CXX_LIST "${TEST_PACKAGE}/${TEST_FILE_CXX}")
          endforeach()
          foreach (TEST_FILE_HXX ${TEST_FILES_HXX})
            list (APPEND TEST_FILES_HXX_LIST "${TEST_PACKAGE}/${TEST_FILE_HXX}")
          endforeach()
        endif()
      endforeach()
    endif()
  ...
  if (TEST_PACKAGE_LIST)
    list (APPEND TEST_TOOLKIT_LIST "${OCCT_TOOLKIT}")
  endif()
  set (TEST_CXX_LIST ${TEST_FILES_CXX_LIST}  PARENT_SCOPE)
  set (TEST_HXX_LIST ${TEST_FILES_HXX_LIST}  PARENT_SCOPE)
  set (TEST_PACKAGE_LIST ${TEST_PACKAGE_LIST} PARENT_SCOPE)
  set (TEST_TOOLKIT_LIST ${TEST_TOOLKIT_LIST} PARENT_SCOPE)
endfunction()

function (OCCT_GET_MODULE_DOC_NAME THE_MODULE_NAME)
  set (VAS_FILE "${CMAKE_SOURCE_DIR}/src/VAS/${THE_MODULE_NAME}.tcl")
  set (MODULE_DOC_NAME)

  if (EXISTS "${VAS_FILE}")
    file (READ "${VAS_FILE}" VAS_MODULE_CONTENT)
    string (REGEX MATCH "proc[ ]+${THE_MODULE_NAME}:documentation_name[ ]+{[ ]+}[ ]+{[\n ]+return[ ]+([^\n]+)" CONTAIN_DOC_NAME "${VAS_MODULE_CONTENT}")
    if ( CONTAIN_DOC_NAME )
      set (MODULE_DOC_NAME "${CMAKE_MATCH_1}")
    endif()
  endif()
  set (MODULE_DOC_NAME ${MODULE_DOC_NAME} PARENT_SCOPE)
endfunction()
